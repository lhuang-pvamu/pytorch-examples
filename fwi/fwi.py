...
def ricker(t, config):
    ...
    # Ricker Wavelet: A = (1-2*pi^2*f^2*t^2)*e^(-pi^2*f^2*t^2)
    t0 = 6.0 / (np.pi * nu0 * np.sqrt(2.0))
    tau = t - t0
    w = (1.0 - 2.0 * (np.pi**2) * (nu0**2) * (tau**2))* np.exp(-(np.pi**2) * (nu0**2) * (tau**2))
...
def point_source(value, position, config):
    ...
    nx = config['nx']
    space = np.linspace(x_limits[0], x_limits[1], nx)
    f = value * (space - position)
    return f
...
f = point_source(1*config['dx'], config['x_s'], config)
plt.figure()
plt.plot(f)
plt.show()
